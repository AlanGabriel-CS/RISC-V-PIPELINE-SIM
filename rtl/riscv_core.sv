module riscv_core (
    ...
    // --- I-cache address freeze (the actual power-saving mechanism) ---
    // instruction_mem's read is purely combinational (no clock to gate), so
    // "clock-gating the I-cache" is modeled here as freezing the address fed
    // to it instead: that's what stops the memory array's decoder/bitlines
    // from switching every cycle, which is the real source of the dynamic
    // power a clock-gated read register would also be saving. gate_icache_clk
    // is driven by loop_buffer below (forward-referenced here -- no
    // combinational loop, since it's a registered signal derived from
    // loop_buffer's FSM state, not from instr_if).
    logic        gate_icache_clk;
    logic [31:0] icache_frozen_addr_q;
    logic [31:0] icache_addr_mux;

    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n)
            icache_frozen_addr_q <= 32'd0;
        else if (!gate_icache_clk)
            icache_frozen_addr_q <= pc_if;   // track live PC whenever not gated
        // else: hold -- gated, so this register stops updating
    end

    // Pure combinational passthrough when not gated (zero added latency,
    // identical timing to the pre-loop-buffer design); switches to the
    // frozen address only while gate_icache_clk is asserted. Since
    // loop_buffer's own instruction_out mux is guaranteed to ignore
    // instruction_in on every cycle gate_icache_clk is high (both are
    // driven by the same STATE_LOOPING condition), the frozen/stale read
    // this produces is never actually consumed -- it exists purely to stop
    // switching activity on the address bus, not to serve real data.
    assign icache_addr_mux = gate_icache_clk ? icache_frozen_addr_q : pc_if;
